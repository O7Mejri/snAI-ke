...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, settings, show=True, fps=100):
        super().__init__()
        ...
        self.border = (0, 40, 0, 40)  # Left, Top, Right, Bottom
        ...
        self._snake_widget_width = max(self.snake_widget_width, 800)
        self._snake_widget_height = max(self.snake_widget_height, 800)
...
class SnakeWidget(QtWidgets.QWidget):
    def __init__(self, parent, board_size=(80, 80), snake=None):
        super().__init__(parent)
        self.board_size = board_size
        self.setAutoFillBackground(True)
        palette = self.palette()
        palette.setColor(self.backgroundRole(), QtGui.QColor(20,20,20))
        self.setPalette(palette)
        if snake:
            self.snake = snake
        self.setFocus()
    ...
    def draw_snake(self, painter: QtGui.QPainter) -> None:
        ...
        brush = QtGui.QBrush()
        idx = 0
        brush.setColor(QtGui.QColor(colors[idx][0], colors[idx][1], colors[idx][2]))
        painter.setBrush(QtGui.QBrush(QtGui.QColor(colors[idx][0], colors[idx][1], colors[idx][2])))
        # painter.setBrush(brush)

        for point in self.snake.snake_array:
 
            brush.setColor(QtGui.QColor(colors[idx%7][0], colors[idx%7][1], colors[idx%7][2]))
            painter.setBrush(QtGui.QBrush(QtGui.QColor(colors[idx][0], colors[idx][1], colors[idx][2])))
...
def save_stats(self, population: Population, path_to_dir: str, fname: str):
